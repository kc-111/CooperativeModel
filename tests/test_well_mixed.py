...
from CooperativeModel.config import (
    GridConfig, ModelParameters, DiffusionConfig, SolverConfig,
    SimulationConfig,
)
from CooperativeModel.kinetics import compute_reaction_rates
from CooperativeModel.model import simulate, BioreactorRHS
...
def _make_config(grid, t_final=2.0, n_output=21, rtol=1e-7, atol=1e-9):
    ...
    return SimulationConfig(
        ...
        diffusion=DiffusionConfig(
            D_N1=0.0, D_N2=0.0, D_Sn=0.0, D_L=0.0,
            D_F1=0.0, D_F2=0.0, D_F3=0.0, D_F4=0.0,
        ),
...
def _solve_ode(y0, params, t_final, n_output, rtol=1e-9, atol=1e-12):
    ...
    ``y0`` is ``[B, 8]``; returned ``y`` is ``[B, n_output, 8]``.
    """
    B = y0.shape[0]

    def rhs(t, y_flat, args=None):
        y_4d = y_flat.reshape(B, 8, 1, 1, 1).clamp(min=0.0)
        return compute_reaction_rates(y_4d, params).reshape(B, -1)
...
def test_spec_well_mixed_matches_0D():
    ...
    ic = uniform(grid, N1=0.05, N2=0.05, Sn=0.0, L=0.0,
                 F1=25.0, F2=25.0, F3=25.0, F4=25.0,
                 device=cfg.device, dtype=cfg.dtype)  # [1, 8, 1, 1, 1]

    results, t_eval = simulate(cfg, ic, velocity_field=None, wall_mask=None)
    # results: [1, n_output, 8, 1, 1, 1]
    pde = results.reshape(1, -1, 8)
...
def test_locality_per_voxel_matches_0D():
    ...
    torch.manual_seed(0)
    ic = torch.zeros(1, 8, grid.Nz, grid.Ny, grid.Nx, dtype=torch.float64)
    # Per-channel non-uniform IC.
    ic[0, 0] = 0.05 + 0.02 * torch.rand(grid.Nz, grid.Ny, grid.Nx, dtype=torch.float64)
    ic[0, 1] = 0.05 + 0.02 * torch.rand_like(ic[0, 1])
    # Sn, L start at 0.
    for ch in (4, 5, 6, 7):
        ic[0, ch] = 20.0 + 10.0 * torch.rand_like(ic[0, ch])

    results, _ = simulate(cfg, ic, velocity_field=None, wall_mask=None)
    # results: [1, n_output, 8, 4, 4, 4]

    # Reference: solve a 0D ODE per voxel in one batched solve.
    n_vox = grid.Nz * grid.Ny * grid.Nx
    y0 = ic.permute(0, 2, 3, 4, 1).reshape(n_vox, 8)
    params = cfg.model.to_tensors(device=cfg.device, dtype=cfg.dtype)
    ode = _solve_ode(y0, params, cfg.solver.t_final, cfg.solver.n_output)
    # ode: [n_vox, n_output, 8] -> [1, n_output, 8, Nz, Ny, Nx]
    ode_grid = (ode.reshape(grid.Nz, grid.Ny, grid.Nx, cfg.solver.n_output, 8)
                   .permute(3, 4, 0, 1, 2)
                   .unsqueeze(0))
...
def test_conservation_uniform_field_zero_diffusion(tmp_path):
    ...
    F_const = 25.0
    ic = uniform(grid, N1=0.0, N2=0.0, Sn=0.0, L=0.0,
                 F1=F_const, F2=F_const, F3=F_const, F4=F_const,
    ...
    # results: [1, n_output, 8, Nz, Ny, Nx]

    # F1-F4 have no kinetics source in the absence of bacteria (N1=N2=0).
    # Their total mass over fluid cells must therefore be conserved by the
    # advection operator alone, to within ODE-solver tolerance.
    fluid_b = fluid > 0.5
    for ch in (4, 5, 6, 7):
